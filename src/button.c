...
Button init_button(vec2 pos, vec2 size, const char *label){
    ...
    new_button.color = (vec4){0, 255, 0 , 255};
    new_button.hover_color = (vec4){155, 225, 155, 255};
...
void render_states_current_button(GameContext *cxt){
    ...
    for(int i = 0; i < current_state->button_count; i++){
        ...
        if(current_button->button_flags.is_hovering)
            ...
            SDL_SetRenderDrawColor(cxt->app.renderer, current_button->color.r, current_button->color.g,
                                   current_button->color.b, current_button->color.a);
